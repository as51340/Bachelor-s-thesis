...
public class FPGAModel {
	...
	public FPGAModel(int rows, int columns, int clbVariables, int wiresCount, int pinsPerSegment) {
		...
		for(int y = 0; y <= rows; y++) {
			for(int c = 0; c < columns; c++) {
				...
				for(int i = 0; i < wiresCount; i++) {
					WireSegment w = new WireSegment(switchBoxes[swIndex], wiresCount+i, switchBoxes[swIndex+1], 3*wiresCount+i, y, c, 
							true, i); //i je index, redni broj žice, firstIndex prema desno, secondIndex prema lijevo => gledajući u odnosu na switch boxeve, ali zar nije onda opet krivi pristup ako se gleda naopačke
					wiresList.add(w);
				}
			}
			swIndex++;
		...
		for(int x = 0; x <= columns; x++) {
			for(int r = 0; r < rows; r++) {
				...
				for(int i = 0; i < wiresCount; i++) {
					WireSegment w = new WireSegment(switchBoxes[swIndex], 2*wiresCount+i, switchBoxes[swIndex+columns+1], i, r, x, false, i); //firstIndex se gleda prema dolje, second index prema gore
					wiresList.add(w);
				}
			}
		}
		...
		for(int r = 0; r < rows; r++) {
			for(int c = 0; c < columns; c++) {
				...
				for(int i = 0; i < wiresCount; i++) {
					clbs[clbIndex].wiresIn[i] = switchBoxes[swIndex].segments[2*wiresCount+i]; //uzmi input s lijevog switch boxa i njegov wire segment(kako je to točno id) to su valjda wire segmenti ispod nekog switch boxa
					clbs[clbIndex].wiresOut[i] = switchBoxes[swIndex+1].segments[2*wiresCount+i]; //output spoji na desni switch box i njegov donji wire segment ako sam u pravu
				}
			}
		}
		...
		for(int c = 0; c < columns; c++) {
			...
			for(int p = 0; p < pinsPerSegment; p++) {
				pinIndex++;
				for(int i = 0; i < wiresCount; i++) {
					pins[pinIndex].wires[i] = switchBoxes[c].segments[wiresCount+i]; //po mojoj računici to je desni wire segment konkretnog switch boxa
				}
			}
		}
		...
		for(int c = 0; c < columns; c++) {
			...
			for(int p = 0; p < pinsPerSegment; p++) {
				pinIndex++;
				for(int i = 0; i < wiresCount; i++) {
					pins[pinIndex].wires[i] = switchBoxes[rows*(columns+1)+c].segments[wiresCount+i]; //pospajaj po zadnjim redovima i spoji se opet na desni wire segment?
				}
			}
		}
		...
		for(int r = 0; r < rows; r++) {
			...
			for(int p = 0; p < pinsPerSegment; p++) {
				pinIndex++;
				for(int i = 0; i < wiresCount; i++) {
					pins[pinIndex].wires[i] = switchBoxes[r*(columns+1)].segments[2*wiresCount+i]; //spoji se na donji wire segment
				}
			}
		}
		...
		for(int r = 0; r < rows; r++) {
			...
			for(int p = 0; p < pinsPerSegment; p++) {
				pinIndex++;
				for(int i = 0; i < wiresCount; i++) {
					pins[pinIndex].wires[i] = switchBoxes[r*(columns+1)+columns].segments[2*wiresCount+i]; //spoji se na donji wire segment
				}
			}
		}
	}
	...
	public LabelsFillResult fillLabels() { //kad se to poziva, u kojem dijelu?
		LabelsFillResult result = new LabelsFillResult();
		for(Pin pin : pins) { //svaki pin ima referencu na wires. Da li je connection index, redni broj spoja?
			if(pin.connectionIndex != -1 && pin.input) { //ako ima neki spoj i ako je ulazni
				fillLabel(pin, pin.wires[pin.connectionIndex], result); // nešto napravi, a šta?
			}
		}
		for(CLBBox box : clbs) {
			if(box.outConnectionIndex!=-1) {
				fillLabel(box, box.wiresOut[box.outConnectionIndex], result); //mislim da su labele "kvadratići" koji se onda postavljaju, ovo je samo onda za output
			}
		}
		return result;
	}

	public void fillLabel(Object label, WireSegment segment, LabelsFillResult result) { //pin ima referencu na wire segment, a wire segment na labelu
		...
		while(!newSet.isEmpty()) {
			...
			if(currentSegment.label==null || currentSegment.label.equals(label)) { //ekvivalentno pitanju da li je već label postavljen tom wire segmentu
				currentSegment.label=label;
			} else {
				currentSegment.label = WireSegment.MULTIPLE_LABELS; //postavi da taj wire segment onda ima više labela?
				result.cumulativeMultiples++; //nešto zbroji?
				multipleFound = true; //???
			}
	...
	private void addSwitchedWires(List<WireSegment> newSet, Set<WireSegment> visitedSet, SwitchBox box, int index) { //u visited setu su svi već pogledani wire segmenti, u newSet dodajemo
		byte[] rowToScan = null;
		for(int i = 0; i < 4*wiresCount; i++) {
			if(i==index) continue; //zato jer ne treba bit spojen sam sa sobom
			if(box.configuration[index][i]==1) { //ako je to npr. horizontalni wire segment onda provjeri spoj od njega prema drugim žicama u tom switch boxu
				WireSegment w = box.segments[i];
				if(w != null && !visitedSet.contains(w)) {
					newSet.add(w);
				}
				rowToScan = box.configuration[i]; //uzmi onda sve konekcije od žice prema kojoj smo gledali spoj
				break;
			}
			if(box.configuration[index][i]==2) { //šta znači da je 1 ili 2?
				WireSegment w = box.segments[i];
				if(w != null && !visitedSet.contains(w)) {
					newSet.add(w);
				}
		...
		//non capisco
		if(rowToScan!=null) { //pogledaj sad sve žice od točke prema kojoj smo gledali spoj s time da je configuration morao biti jednak 1
			for(int i = 0; i < 4*wiresCount; i++) {
				if(rowToScan[i]==2) { //ako je neka veza 2 onda uzmi
					WireSegment w = box.segments[i];
					if(w != null && !visitedSet.contains(w)) {
						newSet.add(w);
					}
				} else if(rowToScan[i]==1) { //inače baci error
	...
	public void simplify(FPGAModelConfiguration conf) { //ovo čisti switch boxeve konfiguracije na temelju modela, ali samo sprječava spajanje potpuno ne povezanih segmenata, može se dodati da pazi ako je samo jedan od njih NULL
		//da ne spaja, pitaj?
	...
	public static void main(String[] args) {
		...
		config.switchBoxes[3][4][1] = 1; //zašto su različiti, šta ne bi spojevi trebali biti isti i da li onda treba u 
		//genetskom pazit i na taj obrnuti smjer
		config.switchBoxes[4][12][11] = 2;
		config.switchBoxes[4][11][12] = 1;
		model.loadFromConfiguration(config);
		model.clearWires(); //sets labels to null
		model.fillLabels();
